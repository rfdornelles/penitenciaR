...
carrega_e_aplica <- function (funcao, lista) {

  # se não for colocada a lista, ler todos os arquivos na pasta
  # /data-raw/sisdepen (que é onde são salvos por padrão)
  if (missing(lista)) {

    lista <- fs::dir_info(path = "data-raw/sisdepen/")$path

  }

  lista %>%
    # carregar as bases, com as colunas no formato de texto
    purrr::map(penitenciaR::carregar_base_sisdepen, "text") %>%
    # depois aplicar a função escolhida
    purrr::map_df(
      # usando possibly pois sabemos que alguns anos dão erro
...
arruma_bloco_estabelecimento <- function (base) {

  # carregar as colunas da base que são relativas ao bloco 1
  # elas começam com 1. (...)
base_bloco1 <- base %>%
...
base_bloco1 <- base_bloco1 %>%
  ...
  renomeia_coluna("quantidade_celas_interditadas", "1.3.*n.o aptas$") %>%
...
#' @rdname blocos
#' @export
arruma_bloco_populacao <- function(base) {

  # carregar as colunas da base que são relativas ao bloco 4
...
arruma_bloco_perfilpessoal <- function(base) {
  ...
  return(base_bloco5_a)
  #TODO: separar por sexo

}


# Base 5 b - Perfil criminal ----------------------------------------------
